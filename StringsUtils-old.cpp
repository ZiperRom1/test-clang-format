...
namespace frlis::utilities {
    ...
    bool StringsUtils::isMatchRefExt(const QString& str) {
        if (!str.isEmpty()) {
            return frlis::utilities::REF_EXT_REGEX.exactMatch(str);
        }
        return false;
    }

    bool StringsUtils::isMatchRefExt(const std::string& str) {
        return isMatchRefExt(QString::fromStdString(str));
    }

    bool StringsUtils::isMatchSexe(const QString& str) {
        if (!str.isEmpty()) {
            return frlis::utilities::SEXE_REGEX.exactMatch(str);
        }
        return false;
    }

    bool StringsUtils::isMatchSexe(const std::string& str) {
        return isMatchSexe(QString::fromStdString(str));
    }

    bool StringsUtils::isMatchVille(const QString& str) {
        if (!str.isEmpty()) {
            return frlis::utilities::VILLE_REGEX.exactMatch(str);
        }
        return false;
    }

    bool StringsUtils::isMatchVille(const std::string& str) {
        return isMatchVille(QString::fromStdString(str));
    }

    bool StringsUtils::isMatchDigit(const QString& str) {
        if (!str.isEmpty()) {
            return frlis::utilities::DIGIT_REGEX.exactMatch(str);
        }
        return false;
    }

    bool StringsUtils::isMatchDigit(const std::string& str) {
        return isMatchDigit(QString::fromStdString(str));
    }

    bool StringsUtils::isMatchDigitAndAsterisc(const QString& str) {
        if (!str.isEmpty()) {
            return frlis::utilities::DIGIT_ASTERISC_REGEX.exactMatch(str);
        }
        return false;
    }

    bool StringsUtils::isMatchDigitAndAsterisc(const std::string& str) {
        return isMatchDigitAndAsterisc(QString::fromStdString(str));
    }

    bool StringsUtils::isMatchLabmedica(const QString& str) {
        if (!str.isEmpty()) {
            return frlis::utilities::LABMEDICA_REGEX.exactMatch(str);
        }
        return false;
    }

    bool StringsUtils::isMatchLabmedica(const std::string& str) {
        return isMatchLabmedica(QString::fromStdString(str));
    }

    bool StringsUtils::isMatchNumSsv(const QString& str) {
        if (!str.isEmpty()) {
            return frlis::utilities::NUM_SSV_REGEX.exactMatch(str);
        }
        return false;
    }

    bool StringsUtils::isMatchNumSsv(const std::string& str) {
        return isMatchNumSsv(QString::fromStdString(str));
    }

    bool StringsUtils::isMatchAlphanumeric(const QString& str) {
        if (!str.isEmpty()) {
            return frlis::utilities::ALPHANUMERIC_REGEX.exactMatch(str);
        }
        return false;
    }

    bool StringsUtils::isMatchAlphanumeric(const std::string& str) {
        return isMatchAlphanumeric(QString::fromStdString(str));
    }

    bool StringsUtils::isMatchAlphanumericAndAsterisc(const QString& str) {
        if (!str.isEmpty()) {
            return frlis::utilities::ALPHANUMERIC_ASTERISC_REGEX.exactMatch(str);
        }
        return false;
    }
    ...
    bool StringsUtils::isMatchEmail(const QString& str) {
        if (!str.isEmpty()) {
            return frlis::utilities::EMAIL_REGEX.exactMatch(str);
        }
        return false;
    }

    bool StringsUtils::isMatchEmail(const std::string& str) {
        return isMatchEmail(QString::fromStdString(str));
    }

    bool StringsUtils::isMatchNumeric(const QString& str) {
      if (!str.isEmpty()) {
          return frlis::utilities::NUMBER_REGEX.exactMatch(str);
      }
      return false;
    }

    bool StringsUtils::isMatchNumeric(const std::string& str) {
        return isMatchNumeric(QString::fromStdString(str));
    }

    bool StringsUtils::isMatchDate(const QString& str) {
      if (!str.isEmpty()) {
          return frlis::utilities::DATE_REGEX.exactMatch(str);
      }
      return false;
    }

    bool StringsUtils::isMatchDate(const std::string& str) {
        return isMatchDate(QString::fromStdString(str));
    }

    bool StringsUtils::isMatchHour(const QString& str) {
      if (!str.isEmpty()) {
          return frlis::utilities::HOUR_REGEX.exactMatch(str);
      }
      return false;
    }

    bool StringsUtils::isMatchHour(const std::string& str) {
        return isMatchHour(QString::fromStdString(str));
    }

    bool StringsUtils::isMatchHourContainingH(const QString& str) {
      if (!str.isEmpty()) {
          return frlis::utilities::HOUR_REGEX_WITH_H.exactMatch(str);
      }
      return false;
    }

    bool StringsUtils::isMatchHourContainingH(const std::string& str) {
        return isMatchHourContainingH(QString::fromStdString(str));
    }
    ...
    std::string StringsUtils::escape(const std::string &s) {
        return boost::replace_all_copy(s, "'", "''");
    }

    std::string StringsUtils::quote(const std::string &s) {
        return "'" + boost::replace_all_copy(s, "'", "''") + "'";
    }

    std::string StringsUtils::replaceJoker(const std::string &s) {
        return boost::replace_all_copy(s, "*", "%");
    }
    ...
    QRegExp StringsUtils::getNumberSqlRegex() {
        return frlis::utilities::NUMBER_SQL_REGEX;
    }

    QString StringsUtils::getDateFormat() {
        return frlis::utilities::DATE_FORMAT;
    }

    QString StringsUtils::getDateTimeFormat() {
        return frlis::utilities::DATE_TIME_FORMAT;
    }

    QString StringsUtils::getDateSqlFormat() {
        return frlis::utilities::DATE_SQL_FORMAT;
    }
    ...
    void StringsUtils::requiredArray(const QJsonObject &params, const QString &key) {
        if (!params.contains(key)) {
            ...
            QStringLiteral("The service expects argument [%1] to be specified").arg(key).toStdString());
        }
        if (!params.value(key).isArray()) {
            throw std::runtime_error(
            QStringLiteral("The service expects argument [%1] to be an array").arg(key).toStdString());
        }
    }
    ...
    void StringsUtils::requiredNumberArray(const QJsonObject &params, const QString &key) {
        ...
        if (std::any_of(arrayValue.begin(), arrayValue.end(), [](const QJsonValue &value)
    ...
    void StringsUtils::optionalNumberArray(const QJsonObject &params, const QString &key, bool allowNull /* = false */) {
        ...
        const auto& arrayValue = params.value(key).toArray();
        if (std::any_of(arrayValue.begin(), arrayValue.end(), [&key](const QJsonValue &value)
        { return !value.isDouble(); }))
            throw std::runtime_error(
                    QStringLiteral("The service expects argument [%1] to be an array of numbers").arg(key).toStdString());
    }
    ...
    static QString getSignSymbol(int64_t x) {
        return x >= 0 ? "" : "-";
    }

    QString StringsUtils::getFormattedMoneyString(
            double doubleValue
    ) {
    ...
    std::vector<std::string> StringsUtils::tokenize(const std::string& input, char seperator, char escape) {
        ...
        if (inEsc){
            throw std::invalid_argument("Input string is invalid (terminating with escape char)");
        }
        if(!token.empty()){
            output.push_back(token);
        }
    ...
    QString StringsUtils::substituteAccents(const QString& str) {
        const static QList<QString> accents  = {"ä", "â", "à", "é", "è", "ê", "ë", "ï", "î", "ì", "ö", "ô", "ò", "ü", "û", "ù", "ç", "À", "Â", "Ä", "È", "É", "Ê", "Ë", "Ì", "Î", "Ï", "Ò", "Ô", "Ö", "Ù", "Û", "Ü", "Ç"};
        const static QList<QChar> substitute = {'a', 'a', 'a', 'e', 'e', 'e', 'e', 'i', 'i', 'i', 'o', 'o', 'o', 'u', 'u', 'u', 'c', 'A', 'A', 'A', 'E', 'E', 'E', 'E', 'I', 'I', 'I', 'O', 'O', 'O', 'U', 'U', 'U', 'C'};
        QString temp = str;
